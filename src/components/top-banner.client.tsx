...
export function TopBanner() {
  ...
  const [msgLong, msgShort, msgEnd] = isPortfolio
    ? ["Chat with Patricia's AI Portfolio Assistant", 'Chat with AI Assistant', 'now']
    : ["Check Patricia's full portfolio", 'See the portfolio', 'here']
  ...
  return (
    <div className="fixed top-0 left-0 right-0 z-[60] h-8 bg-accent/80 flex items-center justify-center px-4">
      ...
      >
        <span className="hidden sm:inline">{msgLong}</span>
        <span className="sm:hidden">{msgShort}</span>
        {' '}<span className="underline underline-offset-2">{msgEnd}</span>

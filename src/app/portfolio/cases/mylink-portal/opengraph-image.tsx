...
const c = CASES_META.find((x) => x.slug === 'mylink-portal')!
      ...
      <div style={{ width: '100%', height: '100%', display: 'flex', background: '#F6F5F0', position: 'relative' }}>
        ...
        <div style={{ display: 'flex', flexDirection: 'column', justifyContent: 'space-between', padding: '64px 80px 64px 96px', width: '100%' }}>
          ...
          <div style={{ display: 'flex', flexDirection: 'column' }}>
            <div style={{ fontFamily: playfairBold ? 'Playfair' : 'serif', fontWeight: 700, fontSize: 64, color: '#131310', lineHeight: 1.1, letterSpacing: '-0.02em', marginBottom: 20, maxWidth: 820, display: 'flex', flexWrap: 'wrap' }}>
              {c.title}
            </div>
            ...
            <div style={{ display: 'flex', alignItems: 'center' }}>
              {tags.map((tag) => (
                <div key={tag} style={{ display: 'flex', alignItems: 'center', background: '#EDECEA', border: '1px solid #E0DFD7', borderRadius: 6, padding: '7px 14px', marginRight: 10 }}>
                  <span style={{ fontFamily: interBold ? 'Inter' : 'sans-serif', fontWeight: 600, fontSize: 12, letterSpacing: '0.06em', textTransform: 'uppercase', color: '#131310', display: 'flex' }}>
                    {tag}
                  </span>
                </div>
              ))}
            </div>
            <span style={{ fontFamily: interBold ? 'Inter' : 'sans-serif', fontWeight: 600, fontSize: 14, color: '#7B7A72', letterSpacing: '0.02em', display: 'flex' }}>
              Patricia Bayona
            </span>
          </div>
        </div>
